...
import { teamsItaly1 } from 'src/data/teams';
...
const restartLeague = async () => {
  // Rounds:
  // const rounds = generateLeagueCalendar(teamsItaly1);
  const rounds = createCalendar(teamsItaly1.length);
...
const onRestartLeague = () => {
  $q.dialog({
    html: true,
    title:
      '<span class="text-primary">Reiniciar Serie A</span> <div style="display: flex; flex-direction: column; justify-content: center; align-items: center;"><img style="width: 30%; height: 30%; margin-top: 15px;" src="/images/leagues/italy1.png" /></div>',
    message:
